...
import { Search, Loader2, ShieldCheck, FileDown, AlertTriangle } from "lucide-react";
...
export default function CheckVisaPage() {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    setTimeout(async () => {
      try {
        const res = await fetch(`/api/status/${encodeURIComponent(passportNumber.trim())}`);
  ...
  const getStatusColor = (status: string) => {
    switch(status.toLowerCase()) {
      ...
      case 'biometrics pending': return 'bg-yellow-100 text-yellow-800 border-yellow-200';
      default: return 'bg-blue-100 text-blue-800 border-blue-200'; // Applied
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 py-12 px-4 sm:px-6 lg:px-8 flex flex-col items-center">
      ...
      <div className="text-center mb-10 mt-8">
         <h1 className="text-4xl font-extrabold text-rts-blue mb-4">Visa & Biometrics Status</h1>
         <p className="text-gray-600 max-w-2xl mx-auto">
           Enter your Passport Number below to check your current application status securely.
         </p>
      </div>
      ...
      <div className="w-full max-w-xl">
        ...
        {isScanning && (
          <div className="flex flex-col items-center justify-center py-10">
            ...
            <p className="mt-6 text-rts-blue font-semibold animate-pulse tracking-widest text-sm">SCANNING SECURE DATABASE...</p>
          </div>
        )}
        ...
        {hasScanned && !isScanning && errorMsg && (
          ...
          >
            ...
            <h3 className="text-lg font-bold text-red-900 mb-1">No Record Found</h3>
        ...
        {hasScanned && !isScanning && result && (
          ...
          >
            <div className="bg-rts-blue px-6 py-4 flex justify-between items-center">
              <h3 className="text-white font-bold text-lg">Applicant Profile</h3>
              <span className={`px-3 py-1 rounded-full text-xs font-bold border uppercase tracking-wide ${getStatusColor(result.status)}`}>
                {result.status}
              </span>
            </div>
            ...
            <div className="p-6">
              <div className="grid grid-cols-2 gap-4 mb-8">
                <div>
                  ...
                  <p className="font-semibold text-gray-900 text-lg">{result.fullName}</p>
                </div>
                <div>
                  <p className="text-sm text-gray-500 mb-1">Passport Number</p>
                  <p className="font-bold text-rts-blue text-lg uppercase tracking-wider">{result.passportNumber}</p>
                </div>
              </div>

              <div>
                <h4 className="font-bold text-gray-900 mb-4 pb-2 border-b border-gray-100 flex items-center">
                  <FileDown className="w-5 h-5 mr-2 text-gray-500" /> Secure Documents
                </h4>
                ...
                {result.documents && result.documents.length > 0 ? (
                  <div className="space-y-3">
                    {result.documents.map(doc => (
                      ...
                      >
                         <span className="font-medium text-gray-700 group-hover:text-rts-blue">{doc.fileName}</span>
